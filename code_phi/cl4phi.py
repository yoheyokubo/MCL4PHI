...
def euclidean_distance_sq(x0, x1):
    return torch.sum((x0 - x1) ** 2, -1)
    ...
    def one_epoch(self, epoch, mode, dataloader, query='none'):
        ...
        with torch.autograd.set_grad_enabled(mode==0):
            for hst_locs, phg_locs, label in dataloader['loader']:    
                ...
                loss, dist = self.criterion(embeds_hst[:,0], embeds_phg[:,0], torch.from_numpy(label).to(self.device))
                ...
                if self.cl_type == '0':
                    ...
                    loss *= bs
                    loss += (euclidean_distance_sq(embeds_hst[:,:,None], embeds_hst[:,None]) + \
                             euclidean_distance_sq(embeds_phg[:,:,None], embeds_phg[:,None])).mean()
                elif self.cl_type == '2':
                    loss += (euclidean_distance_sq(embeds_hst[:,:,None], embeds_hst[:,None]) + \
                             euclidean_distance_sq(embeds_phg[:,:,None], embeds_phg[:,None])).mean()
                elif self.cl_type == '3':
...
class CL4PHIData(Data):
    def __init__(self, data_dir, granularity, data_type='phd', augs=['0:0']):
        ...
        for aug in augs:
            if 'local' in aug:
                self.n_local = int(aug.replace('local', ''))
                with open(os.path.join(data_dir, 'pblks', f'{granularity}_dic_pblks_fcgr_k6.pkl'), 'rb') as f:
                    dic_kmer = pickle.load(f)
                raw_Xs = [self.filter_out_zero(dic_kmer[gid]) for gid in global_ids]
                self.X_local = [torch.from_numpy(self.norm(raw_X))[:,None] for raw_X in raw_Xs]
    ...
    def reload_local(self):
        if self.n_local > 0:
            for i in range(len(self.X)):
                self.X[i,-self.n_local:] = self.X_local[i][random.choices(range(len(self.X_local[i])), k=self.n_local)]

    def filter_out_zero(self, X):
